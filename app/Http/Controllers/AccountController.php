...
class AccountController extends Controller
{
    ...
    */
    ...
    public function statistics(Request $request)
    {
     ...
     /**
     * This function is used to verify profile
     */
    public function verifyid(Request $request, User $user)
    {
        ...
        if ($request->hasFile("frontPage")) {
            $data['frontPage'] = $request->file("frontPage")->store("public/frontPage");
            $data['backPage'] = $request->file("backPage")->store("public/backPage");
            $data['user_id'] = Auth::user()->id;
            $data['status'] = 0;
        }
        
        verify_proof::create($data);

        // User::where("id", "=", Auth::user()->id)->update([
        //     "vfystatus" => 1,
        // ]);
        
        return back()->with('message','Document has been submitted successfully');
    }
