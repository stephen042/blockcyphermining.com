...
class UserController extends Controller
{
    ...
    public function staticPages(Request $request, $name)
    {
        ...
        if ($name == "trading") {
            ...
            return view("home.$name", ["weeksPlan" => $weeksPlan, "monthsPlan" => $monthsPlan, "cryptoPlan" => $cryptoPlan, "nfpPlan" => $nfpPlan, "Plans" => $Plans ,"deposite" => $deposits],);
        }
    }
    ...
    public function verify(Request $request, $name, $id = null){
        ...
        if ($request->method() == "GET") {
            ...
            if ($name == "active") 
            {
                ...
                $data = verify_proof::select("users.firstname", "users.lastname", "users.username", "users.country", "verify_proof.*")
                ->orderBy("verify_proof.created_at", "desc")
                ->leftJoin('users', 'verify_proof.user_id', '=', 'users.id')
                ->get();

                return view("admin.$name-verify", ["data" => $data]);
            }
        }

        if ($name == "approve") {

            $verifydata = verify_proof::where("id", "=", $id)->get()->first();
            $userdata = user::where("id", "=", $verifydata->user_id)->get()->first();

            if ($verifydata->status == 1) {
                return back()->with("error", "This request has been approved previously");
            }

            $result = verify_proof::where("id", "=", $id)->update([
                'status' => 1,
            ]);

            $result = user::where("id", "=", $verifydata->user_id)->update([
                'vfystatus' => 1,
            ]);
            ...
            $details = [
                ...
                "content" => "Hello <b>$userdata->username!</b><br><br>
                            Thanks for your patience.
                            Your profile has been approved successfully.<br>",
                "year" => date("Y"),
                "appMail" => config("app.email"),
                "domain" => config("app.url"),
            ];
            $admindetails4 = [
                ...
                "username" => "Admin",
                "content" => "Hello <b>$userdata->username!</b><br><br>
                             profile has been approved successfully .<br>",
                "year" => date("Y"),
                "appMail" => config("app.email"),
                "domain" => config("app.url"),
            ];
            try {
                Mail::to($userdata->email)->send(new GeneralMailer($details));
                Mail::to(config("app.admin_email"))->send(new GeneralMailer($admindetails4));
            } catch (\Exception $e) {
                // Never reached
            }

            return back()->with('success','Profile Proof has been approved successfully.');
        ...
        if ($name == "decline") {
            ...
            if ($verifydata->status == 2) {
                return back()->with("error", "This request has been declined previously");
            }
            ...
            $details = [
                ...
                "content" => "Hello <b>$userdata->username!</b><br><br>
                            Thanks for your patience.
                            We'er sorry your document was not Approved.<br>
                            please contact use for more information.<br>",
            ...
            $admindetails4 = [
                ...
                "username" => "Admin",
                "content" => "Hello <b>$userdata->username!</b><br><br>
                             profile verification process has been declined successfully .<br>",
                "year" => date("Y"),
                "appMail" => config("app.email"),
                "domain" => config("app.url"),
            ];
            ...
            return back()->with('success','Profile Proof has been declined successfully.');
        }
    }
